...
| 5 | Make comparative analyses on other methods vs ours | ✅ Implemented | **§2.13 Comparative Analysis of Existing Technologies** with **Table 2.2** — **p. 33**. | We added §2.13 with Table 2.2 directly comparing existing payment prediction approaches against our system across key dimensions (algorithm, dataset scale, feature type, and interpretability). |
...
| 8 | Table title on top of tables | ✅ Implemented | Table captions are above each table (e.g., **Table 1.1, p. 10**; **Table 2.2, p. 33**). | We ensured all table captions are placed above their respective tables throughout the manuscript, consistent with ACM formatting requirements. |
