...
def main() -> None:
    ...
    row = conn.execute(
        "SELECT model, parameters FROM experiments WHERE model='two_stage_xgb_ada' LIMIT 1"
    ).fetchone()
    print("Two-stage model:", row[0])
    print("Parameters str:", row[1])

    param_str = str(row[1]).strip('"').strip("'")
    s2_marker = ", stage2="
    ...
    row2 = conn.execute(
        "SELECT model, parameters FROM experiments WHERE model='ada_boost' LIMIT 1"
    ).fetchone()
    print()
    print("Base model:", row2[0])
    print("Parameters str:", row2[1])
    pairs = ast.literal_eval(str(row2[1]).strip('"').strip("'"))
    print("Parsed dict:", dict(pairs))

    # Test ordinal model parse
    row3 = conn.execute(
        "SELECT model, parameters FROM experiments WHERE model='ordinal_ada_boost' LIMIT 1"
    ).fetchone()
    print()
    print("Ordinal model:", row3[0])
    print("Parameters str:", row3[1])
    pairs3 = ast.literal_eval(str(row3[1]).strip('"').strip("'"))
